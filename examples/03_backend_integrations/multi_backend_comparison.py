...
def generate_synthetic_data(num_samples=2000, input_size=20, num_classes=3):
    ...
    for class_idx in range(num_classes):
        ...
        class_data = torch.randn(samples_per_class, input_size) * 0.8 + base_values
...
def _log_epoch_metrics(exp, writer, metrics: dict, epoch: int, model: nn.Module):
...
def run_experiment_with_backend(backend_name: str, project_name: str = "Multi-Backend Comparison"):
    ...
    hyperparams = {"learning_rate": 0.001, "batch_size": 64, "epochs": 12}
    ...
    for epoch in range(12):
        train_loss, train_acc = _train_epoch(model, train_loader, criterion, optimizer, exp, writer, epoch)
...
def _print_comparison_report(results: list[dict], total_time: float):
    ...
    for res in results:
        print(f"{res['backend'].upper():<12} {res['best_val_accuracy']:<15.2f}% " f"{res['execution_time']:<10.2f}")
    ...
    print(f"   🎯 Best Accuracy: {best_accuracy['backend'].upper()} " f"({best_accuracy['best_val_accuracy']:.2f}%)")
    print(f"   ⚡ Fastest Training: {fastest_time['backend'].upper()} " f"({fastest_time['execution_time']:.2f}s)")
    print(f"   📊 Total Execution Time: {total_time:.2f}s")
    print(f"   📈 Accuracy Consistency: ±{accuracy_std:.2f}% (lower is more consistent)")
