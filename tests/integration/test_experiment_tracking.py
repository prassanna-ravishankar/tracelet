...
def test_pytorch_experiment_tracking(temp_dir, mock_data):
    ...
    tracelet.start_logging(exp_name="test_pytorch", project="integration_test", backend="mlflow")
    ...
    for epoch in range(5):
        ...
        writer.add_scalars("train/metrics", {"loss": loss.item(), "learning_rate": 0.01}, epoch)
...
@pytest.mark.skipif(
    not tracelet.frameworks.lightning.LightningFramework._check_lightning(), reason="PyTorch Lightning not installed"
)
@pytest.mark.skipif(not _has_torch, reason="PyTorch not installed")
